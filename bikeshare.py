...
def get_filters():
    ...
    while True:
        month = input('Which month? January, February, March, April, May or June? Enter \'all\' for no month filter.\n').lower()
        if month in months:
            break
    # TO DO: get user input for day of week (all, monday, tuesday, ... sunday)
    while True:
        day = input('Which day? Monday, Tuesday, Wednesday, Thursday, Friday, Saturday or Sunday? Enter \'all\' for no day filter.\n').lower()
        if day in DAYS:
            break
...
def time_stats(df):
    ...
    most_common_start_hour = df['Start Time'].value_counts().idxmax()
...
def display_data(df):
    ...
    for i in range(0, row_length, 5):
        result = input('\nWould you like to examine the particular user trip data? Enter \'yes\' or \'no\'.\n> ')
        if result.lower() != 'yes':
            break
        row_data = df.iloc[i: i + 5].to_json(orient='records', lines=True).split('\n')
        for row in row_data:
            parsed_row = json.loads(row)
            data = json.dumps(parsed_row, indent=2)
            print(data)
