...
def time_stats(df):
    ...
    print('-'*40)
    return common_month, common_day_of_week, popular_hour
...
def station_stats(df):
    ...
    print("\nThis took %s seconds." % (time.time() - start_time))
    print('-'*40)
    return common_start_station, common_end_station, df['Trip']
...
def main():
    while True:
        ...
        # Displays five lines of data if the user agrees  
        count=0

        while True:
            raw_input =input("Would to like to see 5 lines of raw data? Enter yes or no: ")
